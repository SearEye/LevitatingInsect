...
# v1.33.0
# - PySpin enumeration fixed + safe lifetime (no ReleaseInstance errors)
# - "Refresh Cameras" + Device dropdown (OpenCV & PySpin)
# - Restored Stimulus Start/End size + Simulation Mode controls
# - Per-camera Show Preview toggles (previews optional, full-res recording)
# - Robust logging to Desktop\LevitatingInsect-main\logs with end-time filenames
# - Implemented CameraNode.record_clip (fixes "no video written" issues)
# - Safer QImage conversion (no memoryview TypeError)
...
def finalize_logging():
    # Rename .tmp to end timestamp at exit (close handlers first)
    try:
    ...
    try:
        ...
        if os.path.exists(_LOG_TMP_PATH):
            try:
                # If a file with the final name exists (unlikely), append a suffix
...
class Config:
    def __init__(self):
        ...
        self.stim_r0_px=8       # Start radius
        self.stim_r1_px=240     # End radius
        ...
        # Cameras (defaults bias to PySpin Blackfly)
        ...
        self.cam0_width=0; self.cam1_width=0    # 0 = max sensor
...
def _spin_system_release():
    ...
    if _SPIN_REF==0 and _SPIN_SYS is not None:
        ...
        except Exception as e: LOGGER.warning("[PySpin] ReleaseInstance warning: %s", e)
        _SPIN_SYS=None

def spin_enumerate()->List[Dict[str,str]]:
    """Return [{'serial': '20123456', 'model':'Blackfly S', 'display':'PySpin 20123456 — Blackfly S'}]"""
    ...
    try:
        ...
        for i in range(n):
            ...
            try:
                # Use TLDeviceNodeMap (works without Init)
                ...
                out.append({"serial":s_serial, "model":s_model, "display":f"PySpin {s_serial} — {s_model}"})
            except Exception as e:
...
class SpinnakerCamera(BaseCamera):
    ...
    def open(self):
        ...
        try:
            ...
            if chosen is None:
                chosen = lst[0]
                if self.serial:
                    LOGGER.warning("[PySpin] Serial %s not found; using first camera.", self.serial)
            self.cam = chosen; self.cam.Init()
            self.node = self.cam.GetNodeMap(); self.snode = self.cam.GetTLStreamNodeMap()

            # Stream: newest only; more buffers
    ...
    def start_acquisition(self):
        if self.cam and not self._acq:
            ...
            except Exception as e: LOGGER.warning("[PySpin] BeginAcquisition: %s", e)
    ...
    def get_frame(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    fmt = PySpin.PixelFormat_Mono8 if self._mono else PySpin.PixelFormat_BGR8
                    if not self._mono:
...
class CameraNode:
    ...
    def _open_if_needed(self):
        ...
        try:
            if self.backend=="PySpin":
                ...
                if dev.cap is None: self.synthetic=True; self.dev=None; LOGGER.info("[%s] OpenCV index %d not available → synthetic", self.name, idx)
    ...
    def record_clip(self, out_path:str, duration_s:float, fourcc_str:str, async_writer:bool=True)->str:
        """Record full-resolution frames to out_path for duration_s seconds."""
        self._open_if_needed()
        # Determine size (open once if synthetic to pick a sensible size)
        if self.synthetic or self.dev is None:
            size=(640,480)
        else:
...
# -------------------- Device Enumeration (GUI needs this) --------------------
...
class SettingsGUI(QtWidgets.QWidget):
    ...
    def __init__(self,cfg:Config,cam0:CameraNode,cam1:CameraNode):
        ...
        # Top row: preset, probe, refresh cameras
        ...
        for idx, node in enumerate((cam0, cam1)):
            ...
            le_ident.setPlaceholderText("OpenCV index (0/1/…) or PySpin serial (e.g., 20042017)")
            ...
            glb.addWidget(QtWidgets.QLabel("ROI Width (0=max):"),4,1); glb.addWidget(sb_w,4,2)
            glb.addWidget(QtWidgets.QLabel("ROI Height (0=max):"),5,1); glb.addWidget(sb_h,5,2)
    ...
    def set_preview_image(self, cam_idx:int, img_bgr: Optional[np.ndarray]):
        if img_bgr is None:
            ...
            return
        # Convert BGR->RGB and build QImage safely (bytes copy avoids memoryview TypeError)
        h,w,_=img_bgr.shape
        if HAVE_OPENCV:
...
class MainApp(QtWidgets.QApplication):
    ...
    def refresh_devices(self):
        """Populate per-camera device dropdowns with OpenCV indices + PySpin serials."""
        # Enumerate OpenCV (fast, small range)
        ocv_list = enumerate_opencv(6)
        # Enumerate PySpin (safe)
        spin_list = spin_enumerate()
        for idx, box in enumerate(self.gui.cam_boxes):
            cb = box["cb_device"]
            cb.clear()
            # Add a helpful header item (disabled)
            cb.addItem("— Select a device (or type manually) —")
            cb.model().item(0).setEnabled(False)
            # PySpin first
            for item in spin_list:
                cb.addItem(item["display"], {"backend":"PySpin","ident":item["serial"]})
            # OpenCV indices
            for s in ocv_list:
                try:
                    ...
                    idxnum = 0
                cb.addItem(f"{s}", {"backend":"OpenCV","ident":str(idxnum)})
            # Bind selection change
            def on_change(i, box=box):
                data = cb.itemData(i)
    ...
    def update_previews(self):
        if self.in_trial:
            ...
            return
        # Only grab if "Show Preview" is enabled
        for i, node in enumerate((self.cam0, self.cam1)):
    ...
    def loop(self):
        self.gui.lbl_status.setText("Status: Watching for triggers…"); LOGGER.info("[Main] Trigger loop started")
        try:
            while self.running:
