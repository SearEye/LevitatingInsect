# ========================= FlyAPI.py (FlyPy v1.26.0) =========================
"""
FlyPy — Unified Trigger → Cameras + Lights + Looming Stimulus
Scaled GUI + Scrollable content + PySpin detection + Max-FPS probe + robust recording.
"""

__version__ = "1.26.0"

import os, sys, time, csv, atexit, threading, importlib, traceback
from collections import deque
from datetime import datetime
from typing import Optional, Tuple, List

# --- Console encoding to avoid Windows CP issues ---
for _io in ("stdout","stderr"):
    try:
        getattr(sys, _io).reconfigure(encoding="utf-8")
    except Exception:
        pass
...
# ---- Qt GUI (import BEFORE any QtWidgets usage) ----
from PyQt5 import QtWidgets, QtCore, QtGui  # <-- fixes NameError when used below
try:
...
class Config:
    def __init__(self):
        ...
        self.cam0_id = "0"      # OpenCV index OR PySpin serial (or "index:X")
        self.cam1_id = "1"
...
def enum_pyspin() -> List[dict]:
    out = []
    if not HAVE_PYSPIN: return out
    sysm = _spin_system_acquire()
    cams = None
    try:
        cams = sysm.GetCameras() if sysm else None
        ...
        for i in range(n):
            try:
                ...
                out.append({"index": i, "serial": sn, "model": model})
            except Exception:
                out.append({"index": i, "serial": "", "model": ""})
        if cams is not None: cams.Clear()
    except Exception as e:
        print(f"[PySpin enum] error: {e}")
        try:
            if cams is not None: cams.Clear()
        except Exception:
            pass
...
def enum_opencv(max_index: int = 10) -> List[dict]:
    out = []
        ...
        for be in backends:
            try:
                ...
                if cap and cap.isOpened():
                    out.append({"index": i, "name": f"OpenCV index {i}"})
                    break
            except Exception:
                pass
            finally:
...
class OpenCVCamera(BaseCamera):
    def __init__(self, index: int, target_fps: float):
        ...
        self.width = 640; self.height = 480; self.fps = target_fps
        ...
        for be in backends:
            try:
                ...
                if cap and cap.isOpened():
                    ...
                    self.fps = float(cap.get(cv2.CAP_PROP_FPS) or self.target_fps or 60.0)
                    self.cap = cap; return
                if cap: cap.release()
            except Exception: pass
        self.cap = None
...
class PySpinCamera(BaseCamera):
    ...
    def open(self):
        ...
        try:
            ...
            # PixelFormat: prefer Mono8 (fast, grayscale)
        ...
        try:
            ...
            try:
                if img.IsIncomplete():
                    return None
                arr = img.GetNDArray()
                # Convert to BGR for OpenCV writer consistency
                if arr.ndim == 2:
                    if HAVE_OPENCV:
                ...
                return arr  # BGR or 3-channel array
            finally:
                img.Release()
        except Exception:
            return None
...
class CameraNode:
    ...
    def _open_if_needed(self):
        ...
        try:
            if self.backend == "PySpin":
                ...
                # Allow values like "index:1" to reference by index explicitly
                if ident.startswith("index:"):
                    idx = int(ident.split(":",1)[1])
                    # map index -> serial via enumeration
                    cams = enum_pyspin()
                    serial = ""
                    if 0 <= idx < len(cams):
                        serial = cams[idx].get("serial","")
                    dev = PySpinCamera(serial, self.target_fps)
                else:
                    dev = PySpinCamera(ident, self.target_fps)  # blank → first camera
                dev.open()
                self.dev = dev
                self._max_fps_cache = dev.max_fps()
                print(f"[{self.name}] PySpin open: serial={'(auto)' if not ident else ident}, max_fps={self._max_fps_cache}")
            else:
                if not HAVE_OPENCV: raise RuntimeError("OpenCV not installed")
                idx_txt = (self.ident or "0").strip()
                if idx_txt.startswith("index:"):
                    idx = int(idx_txt.split(":",1)[1])
                else:
                    idx = int(idx_txt or "0")
    ...
    def grab_preview(self, w: int = 640, h: int = 480):
        self._open_if_needed()
        if self.synthetic:
            ...
            return frame[:, :, ::-1] if HAVE_OPENCV else frame  # present to GUI as RGB
        if self.dev is None:
            ...
            self.preview_times.append(time.time())
            return frame[:, :, ::-1] if HAVE_OPENCV else frame
        img_bgr = self.dev.get_frame()
        if img_bgr is None:
            ...
            out = cv2.resize(img_bgr, (w, h)) if HAVE_OPENCV else img_bgr
        self.preview_times.append(time.time())
        # Convert BGR -> RGB for the Qt preview label
        if HAVE_OPENCV:
            out = out[:, :, ::-1].copy()
        return out
    ...
    def record_clip(self, path: str, duration_s: float, fourcc: str, async_writer: bool=False):
        """
        Record a fixed-duration clip to disk, robust to color/gray channel mismatches.
        Lazy-create writer on first real frame to infer channels; convert frames as needed.
        """
        ...
        # Default fallback size/fps
        size = (640, 480)
        fps  = float(self.target_fps or 60.0)

        try:
            while time.time() < t_end:
                if self.dev is None and not self.synthetic:
                    time.sleep(0.005); continue

                if self.synthetic:
                    # Generate a simple moving pattern
                    img_bgr = np.full((size[1], size[0], 3), 180, dtype=np.uint8)
                    if HAVE_OPENCV:
                        cv2.putText(img_bgr, f"{self.name} synthetic", (24, 40),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,0,0), 2, cv2.LINE_AA)
                else:
                    img_bgr = self.dev.get_frame()

                if img_bgr is None:
                    # No frame; keep looping without writing
                    time.sleep(0.001); 
                    continue

                # Establish writer based on first frame
                if writer is None:
                    h, w = img_bgr.shape[:2]
                    size = (w, h)
                    ch = 1 if (img_bgr.ndim == 2 or img_bgr.shape[2]==1) else 3
                    is_color = (ch == 3)
                    if HAVE_OPENCV:
                        four = cv2.VideoWriter_fourcc(*fourcc)
                        writer = cv2.VideoWriter(path, four, fps, size, isColor=is_color)
                    else:
                        raise RuntimeError("OpenCV not available for video writing")
                    if not writer or not writer.isOpened():
                        raise RuntimeError(f"VideoWriter failed to open: {path}")

                # Convert to writer-expected format
                # If writer created with color=True, ensure 3-channel BGR; else ensure grayscale
                if writer is not None:
                    if img_bgr.ndim == 2:
                        img_bgr = cv2.cvtColor(img_bgr, cv2.COLOR_GRAY2BGR) if HAVE_OPENCV else np.stack([img_bgr]*3, axis=-1)
                    elif img_bgr.ndim == 3 and img_bgr.shape[2] == 1:
                        img_bgr = np.repeat(img_bgr, 3, axis=2)
                    if img_bgr.shape[1] != size[0] or img_bgr.shape[0] != size[1]:
                        if HAVE_OPENCV:
                            img_bgr = cv2.resize(img_bgr, size, interpolation=cv2.INTER_NEAREST)
        ...
        return {"path": path, "frames": frames, "avg_fps": frames/elapsed}
...
class PopoutPreview(QtWidgets.QDialog):
    ...
    def _tick(self):
        try:
            img = self._src(640, 480) if self._src.__code__.co_argcount >= 2 else self._src()
            if img is None:
                return
            # img is RGB for GUI
            if img.ndim == 2:
...
class TrialRunner:
    ...
    def run_one(self):
        ...
        self.trial_idx += 1
        self.csvw.writerow([now_stamp(), self.trial_idx, (c0 or {}).get("path",""), (c1 or {}).get("path",""), float(self.cfg.record_duration_s),
...
class SettingsGUI(QtWidgets.QWidget):
    ...
    refresh_cameras_clicked = QtCore.pyqtSignal()
    probe_fps_clicked       = QtCore.pyqtSignal()
    ...
    def __init__(self, cfg: Config, cam0, cam1):
        ...
        for b in (self.bt_start, self.bt_stop, self.bt_trig, self.bt_apply, self.bt_refresh, self.bt_probe):
            top.addWidget(b)
        outer.addLayout(top)
        ...
        self.bt_refresh.clicked.connect(self.refresh_cameras_clicked.emit)
        self.bt_probe.clicked.connect(self.probe_fps_clicked.emit)
        ...
        for idx, node in enumerate((self.cam0, self.cam1)):
            ...
            cb_backend = QtWidgets.QComboBox(); cb_backend.addItems(["OpenCV","PySpin"])
            ...
            glb.addWidget(btn_search, 2, 1); glb.addWidget(cb_found, 2, 2); glb.addWidget(btn_apply_found, 3, 2)

            def _search_and_fill(_idx=idx, _cb_backend=cb_backend, _cb_found=cb_found, _le_ident=le_ident):
                backend = _cb_backend.currentText()
                found = []
                try:
                    if backend == "PySpin":
                        for d in enum_pyspin():
                            s = d.get("serial") or ""
                            name = d.get("model", "")
                            _label = f"{s} {name}".strip()
                            found.append((_label, s))
                    else:
                        for d in enum_opencv():
                            ii = d.get("index")
                            name  = d.get("name","")
                            found.append((f"index:{ii} {name}".strip(), f"index:{ii}"))
                except Exception as e:
                    print("[Search] enumerate error:", e)

                _cb_found.blockSignals(True); _cb_found.clear()
                for label, val in found:
                    _cb_found.addItem(label, val)
                _cb_found.blockSignals(False)
                if found:
                    _le_ident.setText(str(found[0][1]))

            def _apply_found(_cb_found=cb_found, _le_ident=le_ident):
                v = _cb_found.currentData()
                if v is not None:
                    _le_ident.setText(str(v))
            ...
            glb.addWidget(lbl_rep, 5, 1, 1, 2)
            btn_apply_cam = QtWidgets.QPushButton("Apply to this camera")
            glb.addWidget(btn_apply_cam, 6, 1, 1, 2)
            i = idx
            btn_apply_cam.clicked.connect(lambda _, ii=i, cb=cb_backend, le=le_ident, sb=sb_fps: self._apply_one(ii, cb, le, sb))
            self.cam_boxes.append({"gb": gb, "preview": preview, "cb_backend": cb_backend, "le_ident": le_ident, "sb_fps": sb_fps, "lbl_rep": lbl_rep, "cb_found": cb_found, "do_search": _search_and_fill})
    ...
    def refresh_dropdowns(self):
        # Trigger the per-camera search callbacks to repopulate
...
def set_preview_image(self, cam_idx: int, img_rgb: np.ndarray):
        """Safely convert NumPy frame to QImage and show in preview label.

        Accepts either HxW (grayscale) or HxWx3 (RGB) uint8 arrays. Falls back to copying
        via .tobytes() so that Qt has an owning buffer and we avoid "memoryview" type errors.
        """
        try:
            if img_rgb is None:
                return
            ...
            if img_rgb.ndim == 2 or (img_rgb.ndim == 3 and img_rgb.shape[2] == 1):
                gray = np.ascontiguousarray(img_rgb.squeeze())
                h, w = gray.shape
                qimg = QtGui.QImage(gray.tobytes(), w, h, w, QtGui.QImage.Format_Grayscale8)
            else:
                rgb = np.ascontiguousarray(img_rgb[..., :3])
                h, w, _ = rgb.shape
                qimg = QtGui.QImage(rgb.tobytes(), w, h, 3*w, QtGui.QImage.Format_RGB888)

            self.prev_pixmap[cam_idx] = QtGui.QPixmap.fromImage(qimg)
            self.prev_lbl[cam_idx].setPixmap(
                self.prev_pixmap[cam_idx].scaled(
                    self.prev_lbl[cam_idx].size(),
                    QtCore.Qt.KeepAspectRatio,
                    QtCore.Qt.SmoothTransformation
                )
            )
        except Exception as e:
            print(f"[GUI] Preview {cam_idx} error: {e}")
...
class MainApp(QtWidgets.QApplication):
    def __init__(self, argv):
        ...
        self.gui.refresh_cameras_clicked.connect(self.refresh_cameras)
        self.gui.probe_fps_clicked.connect(self.probe_max_fps)

        self.show_scaled_gui(self.cfg.gui_screen_index)

        self.running = False; self.in_trial = False; self.thread = None
        self.preview_timer = QtCore.QTimer(self); self.preview_timer.setInterval(250)
    ...
    def _print_startup_summary(self):
        ...
        print(f"PsychoPy: {'OK' if _ensure_psychopy_loaded() else 'not available (OpenCV fallback)'}")
    ...
    # ---- New: Refresh cameras & Max-FPS probe ----
    def refresh_cameras(self):
        # repopulate search dropdowns for both cameras, respecting selected backends
        self.gui.refresh_dropdowns()
        print("[Main] Camera lists refreshed")

    def probe_max_fps(self, duration_s: float = 1.0):
        """Quick probe: sample previews for a second to estimate achievable FPS; show in labels."""
        end_t = time.time() + float(duration_s)
        cnt0 = cnt1 = 0
        t0 = time.time()
        while time.time() < end_t:
            if self.cam0.grab_preview(320, 200) is not None: cnt0 += 1
            if self.cam1.grab_preview(320, 200) is not None: cnt1 += 1
            QtWidgets.QApplication.processEvents()
        dt = max(1e-9, time.time()-t0)
        fps0 = cnt0 / dt; fps1 = cnt1 / dt
        self.gui.update_cam_label(0, fps0, self.cam0.max_fps_cached())
        self.gui.update_cam_label(1, fps1, self.cam1.max_fps_cached())
        print(f"[Probe] ~{fps0:.1f} fps (cam0), ~{fps1:.1f} fps (cam1)")
