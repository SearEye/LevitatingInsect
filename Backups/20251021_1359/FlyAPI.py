...
# v1.34.2
# - FIX: removed processEvents() from worker thread (prevents GUI freeze)
# - FIX: robust try/except in timers so exceptions can't crash Qt loop
# - FIX: PySpin enumeration/open now index-based (no hidden SWIG refs), safer ReleaseInstance
# - FIX: avoid double cleanup; only Qt aboutToQuit triggers resource free
# - Keeps: logging to Desktop\LevitatingInsect-main\logs (finalized with end time)
# - Keeps: "Refresh Cameras", device dropdown (OpenCV & PySpin), simulation mode
# - Keeps: preview toggles off by default, full-res recording only, FPS probe
# - Keeps: stimulus start/end size, fullscreen/screen chooser
...
def excepthook(exctype, value, tb):
    ...
    # Let Python still print default traceback to stderr
...
class Config:
    def __init__(self):
        ...
        # Cameras (defaults bias to PySpin)
    ...
    def open(self):
        ...
        try:
            ...
            if self.serial:
                for i in range(n):
                    try:
                        sn = lst.GetByIndex(i).GetUniqueID()
                    except Exception:
                        sn = None
                    if sn == self.serial:
                        chosen_idx = i; break
    ...
    def get_frame(self):
        ...
        try:
            ...
            try:
                ...
                # We will release after copying out of arr (below)
            if arr.ndim==2:
...
class SettingsGUI(QtWidgets.QWidget):
    ...
    def set_preview_image(self, cam_idx:int, img_bgr: Optional[np.ndarray]):
        if img_bgr is None:
            ...
            self.cam_boxes[cam_idx]["preview"].setPixmap(QtGui.QPixmap())  # clear pixmap
            return
...
class MainApp(QtWidgets.QApplication):
    def __init__(self, argv):
        ...
        self.aboutToQuit.connect(self.cleanup)  # single cleanup path
        ...
        try:
            while self.running:
                if self.hw.check_trigger():
                    ...
                    self.in_trial=False
                    self.gui.lbl_status.setText("Status: Waiting / Idle.")
                # IMPORTANT: do NOT call QApplication.processEvents() here (non-GUI thread)
                time.sleep(0.005)
        finally:
