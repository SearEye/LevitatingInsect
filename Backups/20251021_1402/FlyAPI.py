...
# v1.35.0
# - FIX: PySpin enumerate no longer releases System mid-run (defer to app exit)
# - FIX: clear all SWIG refs during enumeration to avoid interface ref leaks
# - FIX: GUI freeze guards; no processEvents in worker threads
...
def _spin_system_release_deferred():
    """No-op during runtime; real release occurs in app cleanup."""
    return
...
class CameraNode:
    ...
    def record_clip(self, out_path:str, duration_s:float, fourcc_str:str, async_writer:bool=True)->str:
        ...
        if not HAVE_OPENCV:
            # Minimal stub: create a blank video substitute
...
class MainApp(QtWidgets.QApplication):
    ...
    def cleanup(self):
        ...
        # Only now, when all cameras are released, release PySpin System.
