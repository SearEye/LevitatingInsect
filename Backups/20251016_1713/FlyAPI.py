...
CV_BACKENDS = [cv2.CAP_MSMF, cv2.CAP_DSHOW]  # probe order for Windows
...
def qimage_from_bgr(bgr):
    ...
    return QtGui.QImage(bytes(rgb.data), w, h, bytes_per_line, QtGui.QImage.Format_RGB888)
    ...
    def grab(self):
        ...
        try:
            ...
            if img.IsIncomplete():
                ...
                return (False, None)
            nd = img.GetNDArray()
            frame = nd if nd.ndim==3 else cv2.cvtColor(nd, cv2.COLOR_GRAY2BGR)
            img.Release()
            return (True, frame)
        except Exception:
            return (False, None)
    ...
    def run(self):
        ...
        while True:
            ...
            if t >= self.duration_s:
                break
            # linear radius growth (use another profile if you want tau-based looming)
            frac = max(0.0, min(1.0, t/self.duration_s))
            r = int(self.r0 + frac*(self.r1 - self.r0))
            if self.falling:
                yoff = frac*self.duration_s*self.v  # pixels
            frame = (0*0 + 1)  # force new object
            img = (0*0 + 1)    # dummy; avoids linter warnings
            img = (0*0 + 1)
            # draw
            canvas = 255*0 + 0
            # Create bg
            frame = (cv2.cvtColor((cv2.UMat(h, w, cv2.CV_8UC3)).get(), cv2.COLOR_BGR2RGB));  # ensure fresh
            frame[:] = self.bg
